...
class _PlanningDashboardState extends State<PlanningDashboard>
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      final recent = activeJobs.where((job) => job['updatedAt'] != null).toList()
        ..sort((a, b) {
          final aTime = DateTime.tryParse(a['updatedAt']?.toString() ?? '') ?? DateTime(1970);
          final bTime = DateTime.tryParse(b['updatedAt']?.toString() ?? '') ?? DateTime(1970);
          return bTime.compareTo(aTime);
        });
      _recentActivities = recent.map((job) {
        final updatedAt = DateTime.tryParse(job['updatedAt']?.toString() ?? '');
        String timeString = '';
        if (updatedAt != null) {
      ...
      for (final job in activeJobs) {
        ...
        if (job['artworkReceivedDate'] == null ||
            job['artworkReceivedDate'].toString().isEmpty) {
          pendingFields.add({
        ...
        if (job['artworkApprovedDate'] == null ||
            job['artworkApprovedDate'].toString().isEmpty) {
          pendingFields.add({
            'label': 'Artwork Approved Date',
            'value': 'Not yet Updated',
          });
        }
        if (job['shadeCardApprovalDate'] == null ||
            job['shadeCardApprovalDate'].toString().isEmpty) {
          pendingFields.add({
      ...
      for (final job in activeJobs) {
        ...
        if (updatedAt.isAfter(startOfToday)) {
          if (job['artworkReceivedDate'] != null &&
              job['artworkReceivedDate'].toString().isNotEmpty)
            completedToday++;
          if (job['artworkApprovedDate'] != null &&
              job['artworkApprovedDate'].toString().isNotEmpty)
            completedToday++;
          if (job['shadeCardApprovalDate'] != null &&
              job['shadeCardApprovalDate'].toString().isNotEmpty)
            completedToday++;
        ...
        background: Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [AppColors.maincolor.withOpacity(0.05), Colors.white],
            ),
          ),
        ),
      ),
              ...
              itemBuilder: (context, index) {
                ...
                return InkWell(
                  onTap: () {
                    ...
                    final pendingFieldsRaw =
                        _pendingFieldsByJob[nrcJobNo] ?? [];
            ...
            if (_recentActivities.length > 3)
              TextButton(
                onPressed: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => AllRecentActivitiesPage(
